...
const Filters = () => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        const transformedCategories: Category[] = categoriesArray
          ...
          .map((cat: ApiCategory) => {
            // Generate slug from category name (lowercase, replace spaces with hyphens)
            ...
            return {
              id: cat.id,
              title: cat.name,
              href: `/shop/${categorySlug}`, // Use slug in URL path
  ...
  useEffect(() => {
    ...
    if (inStockParam !== null || outOfStockParam !== null || priceParam !== null || ratingParam !== null) {
      setInputCategory({
        inStock: { 
          text: "instock", 
          isChecked: inStockParam !== "false" 
        },
        outOfStock: { 
          text: "outofstock", 
          isChecked: outOfStockParam !== "false" 
        },
        priceFilter: { 
          text: "price", 
          value: priceParam ? Number(priceParam) : 3000 
        },
        ratingFilter: { 
          text: "rating", 
          value: ratingParam ? Number(ratingParam) : 0 
        },
      });
    }
  }, []); // Only run on mount
